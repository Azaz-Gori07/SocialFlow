/**
 * Phase 3 — End-to-End Publishing Integration Test Runner
 *
 * Use this file for execution to avoid ts-node ESM/CommonJS quirks.
 * Run with:
 *   node dist/features/draft/draft.e2e-test.runner.js
 */
...
async function runTests() {
  ...
  // Use in-memory MongoDB if specified
  let mongoServer: any;
  if (process.env.USE_IN_MEMORY_MONGO === 'true') {
    const { MongoMemoryServer } = await import('mongodb-memory-server');
    mongoServer = await MongoMemoryServer.create();
    const uri = mongoServer.getUri();
    // Override the env.MONGO_URI for the mongoose connection
    process.env.MONGO_URI = uri;
    console.log('[INFO] Using in-memory MongoDB:', uri);
  }

  try {
    await cleanup();

    const mongoUri = process.env.MONGO_URI || (env as any).MONGO_URI || 'mongodb://localhost:27017/socialflow_test';
    ...
    if (mongoServer) {
      ...
      console.log('[INFO] In-memory MongoDB stopped.');
    }
    process.exit(0);
  }
}
