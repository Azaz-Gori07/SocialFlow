...
export const CommentController = {
  ...
  replyToComment: async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
      ...
      const updated = await db.comments.updateOne(
        { _id: commentId },
        { 
          $push: { replies: reply },
  ...
  resolveComment: async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
      ...
      const updated = await db.comments.updateOne({ _id: id }, { $set: { status } });
      return res.json(updated);
    } catch (error) {
  ...
  assignComment: async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
      ...
      const updated = await db.comments.updateOne({ _id: id }, { $set: { assignedTo } });
      return res.json(updated);
    } catch (error) {
