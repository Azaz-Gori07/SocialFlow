...
const generateMockPlatformContent = (prompt: string, platform: SocialPlatform): string => {
...
export const AIController = {
  ...
  repurposeYoutube: async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
  ...
  repurposeBlog: async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });

      const { url } = req.body;
      if (!url) return res.status(400).json({ message: 'Blog URL is required' });
  ...
  getInsights: async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (insights.length === 0) {
        ...
        insights = await db.insights.find({ userId });
      }
