...
exports.DashboardController = {
    getOverview: async (req, res) => {
        try {
            ...
            const analyticsRecords = await db_1.db.analytics.find({ accountId: { $in: accountIds } });
            // Group records by date to find the latest data points
            const latestRecordsByAccount = {};
            const previousRecordsByAccount = {};
    ...
    getPlatformBreakdown: async (req, res) => {
        try {
            const userId = req.user?.id;
            if (!userId)
                return res.status(401).json({ message: 'Unauthorized' });
