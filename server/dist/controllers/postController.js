...
exports.PostController = {
    getPosts: async (req, res) => {
        try {
            ...
            const posts = await db_1.db.posts.find(query);
    ...
    getPostById: async (req, res) => {
        try {
            ...
            if (!userId)
                return res.status(401).json({ message: 'Unauthorized' });
            const { id } = req.params;
            const post = await db_1.db.posts.findOne({ _id: id, userId });
            if (!post) {
    ...
    createPost: async (req, res) => {
        try {
            ...
            const post = await db_1.db.posts.create({
                userId,
                platforms: platforms,
                content,
    ...
    updatePost: async (req, res) => {
        try {
            ...
            const updateFields = {};
            if (platforms)
                updateFields.platforms = platforms;
            if (content)
                updateFields.content = content;
            if (platformContent)
                updateFields.platformContent = platformContent;
            if (media)
            ...
            const updated = await db_1.db.posts.updateOne({ _id: id }, { $set: updateFields });
    ...
    deletePost: async (req, res) => {
        try {
            ...
            await db_1.db.posts.deleteOne({ _id: id });
            // Log activity
            await db_1.db.activityLogs.create({
                userId,
                action: 'POST_DELETED',
    ...
    bulkSchedule: async (req, res) => {
        try {
            ...
            for (const p of posts) {
                ...
                if (!platforms || !content || !scheduledAt)
                    continue;
                const newPost = await db_1.db.posts.create({
                    userId,
                    platforms: platforms,
                    content,
                    platformContent: {},
