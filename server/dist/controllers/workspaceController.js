...
exports.WorkspaceController = {
    ...
    listWorkspaces: async (req, res) => {
        try {
            ...
            if (!userId)
                return res.status(401).json({ message: 'Unauthorized' });
            const memberships = await db_1.db.workspaceMembers.find({ userId });
            const workspacesList = [];
            for (const member of memberships) {
                const workspace = await db_1.db.workspaces.findById(member.workspaceId);
                if (workspace) {
                    workspacesList.push({
                        id: workspace._id,
                        name: workspace.name,
    ...
    inviteMember: async (req, res) => {
        try {
            ...
            const callerMembership = await db_1.db.workspaceMembers.findOne({ workspaceId, userId });
            if (!callerMembership || (callerMembership.role !== 'owner' && callerMembership.role !== 'admin')) {
                return res.status(403).json({ message: 'Permissions denied. Only Owner or Admin can invite members' });
            }
            // Find user to invite
            const userToInvite = await db_1.db.users.findOne({ email: email.toLowerCase() });
            if (!userToInvite) {
                return res.status(404).json({ message: 'No registered user found with this email' });
            }
            // Check if already in workspace
            const existingMember = await db_1.db.workspaceMembers.findOne({ workspaceId, userId: userToInvite._id });
            if (existingMember) {
                return res.status(400).json({ message: 'User is already a member of this workspace' });
            }
            // Add member
            const newMember = await db_1.db.workspaceMembers.create({
                workspaceId,
                userId: userToInvite._id,
                role: role
            });
            // Send notification to the invited user
            const workspace = await db_1.db.workspaces.findById(workspaceId);
    ...
    updateRole: async (req, res) => {
        try {
            ...
            const targetMembership = await db_1.db.workspaceMembers.findOne({ workspaceId, userId: memberUserId });
            if (!targetMembership) {
            ...
            const updated = await db_1.db.workspaceMembers.updateOne({ workspaceId, userId: memberUserId }, { $set: { role } });
    ...
    removeMember: async (req, res) => {
        try {
            ...
            await db_1.db.workspaceMembers.deleteOne({ workspaceId, userId: memberUserId });
    ...
    getWorkspaceMembers: async (req, res) => {
        try {
            ...
            const memberships = await db_1.db.workspaceMembers.find({ workspaceId });
            const membersList = [];
            for (const member of memberships) {
                const userObj = await db_1.db.users.findById(member.userId);
                if (userObj) {
                    membersList.push({
                        userId: userObj._id,
