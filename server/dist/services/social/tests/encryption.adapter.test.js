...
const encryption_adapter_1 = require("../adapters/encryption.adapter");
describe('EncryptionAdapter Unit Tests', () => {
    const plainText = 'secret_token_12345!@#';
    it('should successfully encrypt plain text', () => {
        const encrypted = encryption_adapter_1.EncryptionAdapter.encrypt(plainText);
    ...
    it('should successfully decrypt encrypted text back to original', () => {
        const encrypted = encryption_adapter_1.EncryptionAdapter.encrypt(plainText);
        const decrypted = encryption_adapter_1.EncryptionAdapter.decrypt(encrypted);
        expect(decrypted).toBe(plainText);
    });
    it('should throw error when decrypting invalid format', () => {
        expect(() => {
            encryption_adapter_1.EncryptionAdapter.decrypt('invalidformatwithoutcolon');
        }).toThrow('Invalid encrypted format');
    });
    it('should throw error when decrypting invalid ciphertext hex', () => {
        expect(() => {
            encryption_adapter_1.EncryptionAdapter.decrypt('1234567890abcdef1234567890abcdef:nothex');
        }).toThrow();
    });
});
