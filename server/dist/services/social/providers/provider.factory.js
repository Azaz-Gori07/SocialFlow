...
const youtube_provider_1 = require("./youtube.provider");
...
class ProviderFactory {
    ...
    static getProvider(platform) {
        ...
        const isMockCredentials = (clientId) => {
            return (!clientId ||
                clientId.startsWith('mock_') ||
                clientId.startsWith('your_') ||
                clientId === '');
        };
        const checkRealCredentials = (platformName, clientId) => {
            if (isMockCredentials(clientId)) {
                if (!this.allowMock()) {
                    throw new Error(`[ProviderFactory] ${platformName} Client ID is not configured. ` +
        ...
        switch (cleanPlatform) {
            case 'twitter':
            case 'x':
                checkRealCredentials('Twitter', env_config_1.env.X_CLIENT_ID);
                if (isMockCredentials(env_config_1.env.X_CLIENT_ID)) {
                ...
                return new twitter_provider_1.TwitterProvider();
            case 'linkedin':
                checkRealCredentials('LinkedIn', env_config_1.env.LINKEDIN_CLIENT_ID);
                if (isMockCredentials(env_config_1.env.LINKEDIN_CLIENT_ID)) {
                ...
                return new linkedin_provider_1.LinkedInProvider();
            case 'youtube':
            case 'google':
                checkRealCredentials('Google/YouTube', env_config_1.env.GOOGLE_CLIENT_ID);
                if (isMockCredentials(env_config_1.env.GOOGLE_CLIENT_ID)) {
                ...
                return new youtube_provider_1.YouTubeProvider();
            default:
